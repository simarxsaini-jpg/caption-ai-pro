...
// Serve static files - handle both 'public' and 'Public'
const fs = require('fs');
...
console.log('Serving static from:', staticFolder);
...
app.get('/', (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.status(404).send('index.html not found. Folder: ' + staticFolder + ' Files: ' + fs.readdirSync(__dirname).join(', '));
  }
});
...
// Extract captions from AI response
...
app.listen(PORT, () => {
  ...
  console.log('Files in root:', fs.readdirSync(__dirname).join(', '));
});